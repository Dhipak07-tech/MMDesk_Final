...
export function Leaderboard() {
 ...
 <div>
 <h1 className="text-4xl font-semibold tracking-tight">SLA Performance Leaderboard</h1>
...
"text-red-500"
 ...
 <div>
 <span className="text-xs font-bold text-slate-800">Ticket {ticket.number}</span>
...
function PodiumCard({
 ...
 >
 <div className={`absolute -top-8 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-widest ${styles.bg} ${styles.color} border ${styles.border} whitespace-nowrap`}>
 ...
 <span className={`text-3xl font-semibold ${styles.color}`}>{getInitial(user.name)}</span>
 </div>
 ...
 {/* SLA Score / Breaches */}
 {isBreaches ? (
 <div className="flex items-center justify-center gap-1 font-semibold text-2xl text-red-500">
 <AlertCircle className="w-5 h-5 text-red-500" />
 {user.slaScore}
 </div>
 ) : (
 <div className={`flex items-center justify-center gap-1 font-semibold text-2xl ${styles.color}`}>
