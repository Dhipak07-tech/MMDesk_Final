...
function FeedEntry({ entry, onPreview }: { entry: ActivityEntry; onPreview: (src: string) => void; key?: React.Key }) {
 ...
 <span className="text-xs text-amber-600 dark:text-amber-400">{fmtTime(entry.timestamp)}</span>
 </div>
 ...
 {entry.ticketNumber && (
 <span className="text-xs font-bold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 px-2 py-0.5 rounded-md">{entry.ticketNumber}</span>
 )}
 </div>
 ...
 {entry.confidence > 0 && (
 <div className="ml-auto text-[10px] text-slate-300 dark:text-[#94A3B8]">{Math.round(entry.confidence * 100)}% confidence</div>
 )}
 </div>
 </div>
 </div>
...
function SummaryCard({ summary, duration, entryCount, onDismiss }: { summary: string; duration: number; entryCount: number; onDismiss: () => void }) {
 ...
 <div className="font-bold text-xl">{fmtHMS(duration)}</div>
 </div>
 <div className="bg-white/10 rounded-xl p-3">
 <div className="text-white/60 text-xs uppercase tracking-wide mb-1">Snapshots</div>
 <div className="font-bold text-xl">{entryCount}</div>
 </div>
 </div>
...
export function ActivityTracker() {
 ...
 {isActive && <div className="text-xl font-bold text-green-600 tabular-nums">{fmtHMS(elapsed)}</div>}
 </div>
 ...
 <span className="text-xs text-muted-foreground dark:text-[#94A3B8] w-16 text-right">{pct}% ({count})</span>
 </div>
 );
 })}
 </div>
